...
export class Bomb {
    ...
    static setExplosionEffect(row, col, explosionStageCounter, flames, bombId) {
        ...
        directions.forEach((direction) => {
            ...
            for (let i = 1; i <= flames; i++) {
                ...
                if (
                    !this.arrayContains(this.bombsData[bombId].affectedCells, [
                        r,
                        c,
                    ])
                ) {
    ...
    static constructExplosionElement(key, explosionStageCounter, isWing) {
        ...
        if (key === "center") {
            explosion.attrs.class =
                this.centerExplosionStages[explosionStageCounter];
        } else if (isWing) {
            explosion.attrs.class =
                this.wingExplosionStages[explosionStageCounter];
        } else {
            explosion.attrs.class =
                this.sideExplosionStages[explosionStageCounter];
        }
        if (key !== "center") {
    ...
    static clearExplosionEffect(bombId) {
        ...
        this.bombsData[bombId].affectedCells.forEach(([r, c]) => {
            ...
            if (cell) {
                ...
                const explosionElement = Array.from(cell.children).find(
                    (child) => child.className.startsWith("expl-")
                );
                if (explosionElement) {
